...
//   const stripe = useStripe();
  ...
  const price = localStorage.getItem("cartTotalAmount");
    ...
    const tryRetrievePaymentIntent = (clientSecret, retries = 3) => {
        if (window.Stripe) {
            window.Stripe(import.meta.env.VITE_STRIPE_PUB_KEY).retrievePaymentIntent(clientSecret).then(({ paymentIntent }) => {
            ...
            setTimeout(() => tryRetrievePaymentIntent(clientSecret, retries - 1), 1000);
        } else {
  ...
  return (
    <div className="payment-status-container">
      ...
      {/* You can also render order details here. As an example: */}
      <div className="order-details">
        <h2 className="order-details-title">Order Details</h2>
        {/* For each order item, you can do something like: */}
        
          <div className="order-item" key={order.id}>
            <div className="order-item-header">
              
              <span><label>ID: </label>{order.id}</span>
              <span><label>Date: </label>{order.created}</span>
              <span><label>User Email: </label>{order.userEmail}</span>
            </div>
      ...
      <button className="return-home-btn" onClick={returnToHome}>Return To Home</button>
    </div>
  );
};
